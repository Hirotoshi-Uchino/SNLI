...
def load_test_data():
    test_df = pd.read_csv(TEST_DATA_FILE, sep="\t", usecols=["sentence1", "sentence2", "gold_label"], dtype={"sentence1": str, "sentence2": str, "gold_label": str})
    test_df.fillna("", inplace=True)
    sentence1 = test_df["sentence1"].apply(text_to_tokens) # すべて小文字に
    sentence2 = test_df["sentence2"].apply(text_to_tokens)
    y = test_df["gold_label"].map(get_label_index_mapping()) # 回答ラベルを準備
    tokenizer = Tokenizer(num_words=MAX_NB_WORDS)
    tokenizer.fit_on_texts(sentence1 + sentence2)
    sequences1 = tokenizer.texts_to_sequences(sentence1) # textからvector値へ変換
    sequences2 = tokenizer.texts_to_sequences(sentence2)
...
def run():
    ...
    (X1_test, X2_test, y_test), tokenizer_test = load_test_data()
    Y_test = to_categorical(y_test, num_class)
    
    p_val = model.predict([X1_test, X2_test])
    # ==== evaluation ======
    lll, k = p_val.shape   
    t = 0               
    for i in range(lll):
        if p_val[i].argmax() ==  Y_test[i].argmax():
            t += 1
